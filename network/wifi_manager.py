#!/usr/bin/env python3
"""
Fixed WiFi Manager with Proper State Transitions
...
class FixedWiFiManager:
    ...
    async def _get_device_ssid(self, device: str) -> Optional[str]:
        ...
        try:
            ...
            cmd = ["nmcli", "-t", "-f", "GENERAL.WIFI-PROPERTIES.SSID", "device", "show", device]
            process = await asyncio.create_subprocess_exec(
                *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
    ...
    async def get_available_networks(self) -> list[NetworkInfo]:
        ...
        try:
            ...
            # Get scan results
            cmd = ["nmcli", "-t", "-f", "SSID,SIGNAL,SECURITY,FREQ,IN-USE", "device", "wifi", "list"]
            process = await asyncio.create_subprocess_exec(
                *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
    ...
    async def start_hotspot(self, ssid: str, password: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Hotspot '{ssid}' started successfully at 192.168.4.1")
                return True
            else:
                await self._cleanup_hotspot_connection()
    ...
    async def _create_and_activate_hotspot(self, ssid: str, password: str) -> bool:
        ...
        try:
            ...
            config_commands = [
                ["nmcli", "connection", "modify", self._hotspot_connection_name,
    ...
    async def _perform_network_connection(self, ssid: str, password: str = "") -> bool:
        ...
        try:
            self.logger.info(f"Attempting to connect to '{ssid}' with password: {'***' if password else 'None'}")
            ...
            if process.returncode == 0:
                self.logger.info("nmcli connect command succeeded, waiting for connection to establish...")
                ...
                for attempt in range(max_attempts):
                    ...
                    if status.connected:
                        ...
                        if status.ssid == ssid:
                            self.logger.info(f"Successfully connected to '{ssid}' at {status.ip_address} (attempt {attempt + 1})")
                            return True
                        elif status.ssid:
                ...
                if status.connected and status.ssid != ssid:
                    ...
                    if connection_active:
                        self.logger.info(f"Connection verified by profile check - connected to '{ssid}'")
                        return True
                ...
                if status.connected and status.ssid == ssid:
                    self.logger.info(f"Successfully connected to '{ssid}' at {status.ip_address}")
                    return True
                else:
                    self.logger.warning(f"Connection verification failed after {max_attempts} attempts")
                    self.logger.warning(f"Final status: connected={status.connected}, ssid={status.ssid}")
                    
                    # One more check with device-level verification
                    device_connected = await self._verify_device_connection(ssid)
                    if device_connected:
                        self.logger.info(f"Connection verified by device check - connected to '{ssid}'")
                        return True
                    
                    return False
            else:
                error_msg = stderr.decode().strip()
                self.logger.error(f"nmcli connection failed with exit code {process.returncode}: {error_msg}")
                return False
